...
fn_spawnClearMission = {
	...
	[_newPos, 10, "LOP_AM_Landrover_M2", _ifv1] call BIS_fnc_spawnvehicle;
	...
	waitUntil {{ alive _x } count units _grp1C <= 2 && { alive _x } count units _grp2C <= 2 && { alive _x } count units _grp3C <= 2}; 
...
fn_spawnClear2Mission = {
	...
	waitUntil {{ alive _x } count units _grp1C <= 3 && { alive _x } count units _grp2C <= 3}; 
